        ...
        if st.button('detect'):
            ...
            if model == 'rekog':
                with st.spinner(text='Analysing the video'):
                    video_file = f'yolov5_files/data1/videos/{uploaded_file.name}'
                    analyzeVideo(video_file)
                    
                    json_file = os.path.join("rekog_output", uploaded_file.name.split('.')[0] + ".json")
                    plotting(json_file, video_file)
                    
                    with st.spinner(text='Preparing Video'):
                        path = str(Path('rekog_output') / uploaded_file.name)
                        new_path = path.replace('.mp4', '_new.mp4')
                        video_convert(model, path)
                        st.video(new_path)
