...
def analyzeVideo(videoFile):
    # print(videoFile)
    # videoFile = "C:/Users/M61/PycharmProjects/aws_rekognition/no_smoking_exit_edited.mp4"
    ...
    # plotting the labels onto the original 
    with open(os.path.join("rekog_output", videoFile.split('/')[-1].split('.')[0] + ".json"), 'r') as f:
        data = json.load(f)
    print(len(data))
    ...
    OUTPUT_FILE = os.path.join("rekog_output", videoFile.split('/')[-1].split('.')[0] + ".avi")
...
def main():
    ...
    # vs = cv2.VideoCapture('output_ap_only.avi')
    # while 1: #(vs.isOpened()):
        # (grabbed, img) = vs.read()
        # print(grabbed)
        # if grabbed==True:
            # fourcc = cv2.VideoWriter_fourcc(*"avc1")
            # out = cv2.VideoWriter('output.mp4', fourcc, 30, (img.shape[1], img.shape[0]), True) 
        # out.write(img)
        # print(out.write(img))
    # out.release()
    # vs.release()
    ...
    if source_index == 0:
        ...
        st.write("You can view real-time object detection done using AWS Rekognition here:")
        uploaded_file = st.sidebar.file_uploader("Upload video", type=['mp4'])
        if uploaded_file is not None:
            is_valid = True
            with st.spinner(text='loading...'):
            ...
            if model == 'rekog':
                with st.spinner(text='Analysing the video'):
                    ...
                    with st.spinner(text='Preparing Video'):
                        st.video(os.path.join("rekog_output", uploaded_file.name.split('.')[0] + ".avi"))
                ...
                # runs_print = max(os.listdir('yolov5_files/runs/detect/'),key=os.path.getctime)
